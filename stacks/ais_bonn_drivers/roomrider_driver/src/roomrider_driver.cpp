...
#include "roomrider_driver.h"

RoomriderDriver::RoomriderDriver()
{
  ...
  param_node.param("verbose_mode", param, 0);
  if (param != 0)
  {
    verbose_mode_ = true;
  }
  ROS_DEBUG("Verbose mode: %s",(verbose_mode_)?"true":"false");
  ...
  power_pub_ = node_handle_.advertise<roomrider_driver::Power> ("power", 20);
  lightsensor_pub_ = node_handle_.advertise<roomrider_driver::Lightsensor> ("lightsensor", 20);
  bumper_pub_ = node_handle_.advertise<roomrider_driver::Bumper> ("bumper", 20);
  wheeldrop_pub_ = node_handle_.advertise<roomrider_driver::Wheeldrop> ("wheeldrop", 20);
  infrared_pub_ = node_handle_.advertise<roomrider_driver::Infrared> ("infrared", 20);
  odom_pub_ = node_handle_.advertise<nav_msgs::Odometry> ("odom", 20);
  cmd_sub_ = node_handle_.subscribe<geometry_msgs::Twist> ("cmd_vel", 20, &RoomriderDriver::cmdVelocityCallback, this);
  led_sub_ = node_handle_.subscribe<roomrider_driver::Led> ("led", 20, &RoomriderDriver::ledCallback, this);
}

void RoomriderDriver::mainDeviceLoop()
{
...
/*! Utility function to calculate difference of two wrap-around values
 * here: the encoder count values
 */
int RoomriderDriver::wrapAround16(unsigned short current, unsigned short last)
{
...
// Callback method for motion commands to the roomba
void RoomriderDriver::cmdVelocityCallback(const geometry_msgs::TwistConstPtr& cmd_msg)
{
...
void RoomriderDriver::ledCallback(const roomrider_driver::LedConstPtr& led_msg)
{
...
void RoomriderDriver::handleLeds()
{
...
//! calculates and publishes the odometry
void RoomriderDriver::publishOdometry()
{
...
void RoomriderDriver::publishBumper()
{
  roomrider_driver::Bumper bumper;
...
void RoomriderDriver::publishWheeldrop()
{
  roomrider_driver::Wheeldrop wheeldrop;
...
void RoomriderDriver::publishPower()
{
  roomrider_driver::Power power;
...
void RoomriderDriver::publishLightsensor()
{
  roomrider_driver::Lightsensor sensor_msg;
...
void RoomriderDriver::publishInfrared()
{
  roomrider_driver::Infrared infrared;
  infrared.virtualwall = roomba_dev_->virtual_wall;
  infrared.character_left = roomba_dev_->ir_character_left;
  infrared.character_right = roomba_dev_->ir_character_right;
