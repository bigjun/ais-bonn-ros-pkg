...
int roomba500_open(roomba500_comm_t* r, unsigned char fullcontrol)
{
  ...
  //! Initializing important values for the odometry
...
int roomba500_get_sensors(roomba500_comm_t* r, int timeout)
{
  ...
  /* 100 to get all sensor data, see specification */
  cmdbuf[1] = 100;
...
int roomba500_parse_sensor_packet(roomba500_comm_t* r, unsigned char* buf, size_t buflen)
{
  ...
  r->ir_character_left = buf[idx++];
  r->ir_character_right = buf[idx++];
...
void roomba500_print(roomba500_comm_t* r)
{
  ...
  printf("Ir character left: %d Ir character right: %d\n", r->ir_character_left, r->ir_character_right);
